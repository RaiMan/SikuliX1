 ...
 */
package org.sikuli.script.support.gui.notused;
...
public class PatternPaneNaming extends JPanel {
  ...
  //	public PatternPaneNaming(EditorImageButton imgBtn, JLabel msgApplied) {
  public PatternPaneNaming(JLabel msgApplied) {
    super(new GridBagLayout());
    //init(imgBtn, msgApplied);
  }

  //	private void init(EditorImageButton imgBtn, JLabel msgApplied) {
  private void init(JLabel msgApplied) {
    JLabel lblPath = new JLabel(_I("lblPath"));
    JLabel lblFilename = new JLabel(_I("lblFilename"));

    String filename = ""; //imgBtn.getFilename();
    File f = new File(filename);
    String fullpath = f.getParent();
    filename = getFilenameWithoutExt(f);
    _oldFilename = filename;

    BufferedImage thumb = null; //imgBtn.getThumbnail();
  ...
  public void reloadImage() {
    BufferedImage thumb = null; //_imgBtn.createThumbnailImage(THUMB_MAX_HEIGHT);
    icon.setImage(thumb);
  }
