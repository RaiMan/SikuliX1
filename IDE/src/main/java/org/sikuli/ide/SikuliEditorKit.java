...
public class SikuliEditorKit extends StyledEditorKit {
  ...
  private static final TextAction[] defaultActions = {
          ...
          new NextVisualLineAction(endLineAction, false, SwingConstants.EAST),
          new NextVisualLineAction(beginLineAction, false, SwingConstants.WEST),
          new NextVisualLineAction(selectionEndLineAction, true, SwingConstants.EAST),
...
//TODO up/down might step left or right
  ...
  private static class NextVisualLineAction extends TextAction {
    ...
    NextVisualLineAction(String command, boolean select, int dir) {
      super(command);
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      Caret caret = textArea.getCaret();
      int dot = caret.getDot();
      EditorPane currentCodePane = SikulixIDE.get().getCurrentCodePane();
      ...
      if (!select) {
        switch (direction) {
          case SwingConstants.EAST:
            currentCodePane.jumpTo(lineNumberAtCaret + 1);
            caret.setDot(caret.getDot() - 1);
            return;
          case SwingConstants.WEST:
            currentCodePane.jumpTo(lineNumberAtCaret);
            return;
          default:
        }
      } else {
        switch (direction) {
          case SwingConstants.EAST:
            try {
              int startOffset = currentCodePane.getLineStartOffset(lineNumberAtCaret);
              caret.moveDot(startOffset - 1);
            } catch (BadLocationException e1) {
            }
            return;
          case SwingConstants.WEST:
            try {
              int startOffset = currentCodePane.getLineStartOffset(lineNumberAtCaret - 1);
